...
void C_ConfigManager::setup()
{
	...
	setup_item(&g_cfg.add_shonax.chams_on_shot_enable, false, crypt_str("chams_on_shot.enabled"));
	setup_item(&g_cfg.add_shonax.chams_on_shot_color, Color(255, 255, 255), crypt_str("chams_on_shot.color"));
	setup_item(&g_cfg.add_shonax.chams_on_shot_type, 0, crypt_str("chams_on_shot.type"));
	setup_item(&g_cfg.add_shonax.autostop_fixer, false, crypt_str("shonax_add.autostop_fixer"));
	setup_item(&g_cfg.add_shonax.hitchance_fixer, false, crypt_str("shonax_add.hitchance_fixer"));


	setup_item(&g_cfg.add_shonax.ragdol_force, -1, crypt_str("shonax_add.ragdol_force"));
	setup_item(&g_cfg.add_shonax.ragdol_force_x, -1, crypt_str("shonax_add.ragdol_force_x"));
	setup_item(&g_cfg.add_shonax.ragdol_force_y, -1, crypt_str("shonax_add.ragdol_force_y"));

	setup_item(&g_cfg.add_shonax.autoaccept, false, crypt_str("shonax_add.autoaccept"));
...
void C_ConfigManager::save(std::string config)
{
	...
	auto get_dir = [&folder, &file, &config]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
		{
			...
			file = std::string(path) + crypt_str("\\Plagueware\\Configs\\") + config;
		}
	...
	zfolder = std::string(path) + ("\\Plagueware\\");
	zfolder += ("\\Inventory\\");
	auto zfile = zfolder + "inventory.hack";
...
void C_ConfigManager::pre_safe()
{
	static TCHAR path[MAX_PATH];
	std::string folder;
	SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path);
	folder = std::string(path) + ("\\Plagueware\\");
	folder += ("\\Inventory\\");
	auto zfile = folder + "inventory.hack";
