...
bool ImGui::ContentTab(const char* label, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	// Render
	//const ImVec4 text_dis = ImVec4(202 / 255.f, 202 / 255.f, 202 / 255.f, 1.f); // changes the tabs/text at the top ie "Weapon"
	//const ImVec4 text_act = ImVec4(126 / 255.f, 131 / 255.f, 219 / 255.f, 1.f); // same as above
	//const ImVec4 text_hov = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 1.f);
	const ImVec4 text_dis = ImVec4(0.5f, 0.f, 0.f, 1.f); // changes the tabs/text at the top ie "Weapon"
	const ImVec4 text_act = ImVec4(1.f, 0.f, 0.f, 1.f); // same as above
	const ImVec4 text_hov = ImVec4(0.85f, 0.f, 0.f, 1.f);
...
bool ImGui::MainTab(const char* label, ImFont* icon_font, const char* icon, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y < window->DC.CurrLineTextBaseOffset) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn't be a flag)
		pos.y += window->DC.CurrLineTextBaseOffset - style.FramePadding.y;
	ImVec2 size = CalcItemSize(size_arg, icon_size.x + style.FramePadding.x * 2.0f, icon_size.y + (label_size.y * 2) + style.FramePadding.y * 2.0f);
	...
	const ImVec4 text_dis = ImVec4(79 / 255.f, 8 / 255.f, 160 / 255.f, 1.f);
	const ImVec4 text_act = ImVec4(79 / 255.f, 8 / 255.f, 160 / 255.f, 1.f);

	//const ImVec4 icon_dis = ImVec4(202 / 255.f, 202 / 255.f, 202 / 255.f, 1.f);
	const ImVec4 icon_dis = text_dis;
	const ImVec4 icon_act = ImVec4(79 / 255.f, 8 / 255.f, 160 / 255.f, 1.f); // The icon tab next to rage 
	...
	float mmm2 = 0.33f * 0.9;

	/*PushStyleColor(ImGuiCol_Text, it_icon->second);
	PushFont(icon_font);
	if (icon_font == c_menu::get().ico_csd && icon == crypt_str("D")) {
		RenderTextClipped(bb.Min, ImVec2(bb.Max.x, icon_size.y), icon, NULL, &icon_size, ImVec2(0.6f, 0.0f), &bb);
	} else {
		RenderTextClipped(bb.Min + ImVec2(size_arg.x * mmm2, 0.f), ImVec2(bb.Max.x, icon_size.y), icon, NULL, &icon_size, ImVec2(0.f, 0.0f), &bb);
	}
	PopFont();
	PopStyleColor();*/

	PushStyleColor(ImGuiCol_Text, it_text->second);
	//RenderTextClipped(bb.Min, bb.Max, label, NULL, &label_size, ImVec2(0.5f, 1.f), &bb);
	RenderTextClipped(bb.Min, bb.Max, label, NULL, &label_size, ImVec2(0.5f, 0.5f), &bb);
	PopStyleColor();
...
bool ImGui::BlssButtonXD(const char* label, const ImVec2& size_arg, bool inuse, ImGuiWindowFlags flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	if (g.LastActiveId == g.CurrentWindow->GetID(label))
	{
		...
		t = inuse ? (t_anim) : (1.0f - t_anim);
	}

	ImU32 col_bg;
	col_bg = ImGui::GetColorU32(ImLerp(ImVec4(145 / 255.f, 144 / 255.f, 148 / 255.f, 255.f / 255.f), ImVec4(69 / 255.f, 8 / 255.f, 160 / 255.f, 255.f / 255.f), t));
	ImU32 col_bg2;
	col_bg2 = ImGui::GetColorU32(ImLerp(ImVec4(0.f, 0.f, 0.f, 0.f), ImVec4(0.f, 0.f, 0.f, 80.f / 255.f), t));

	if (inuse) {
		window->DrawList->AddRectFilled(bb.Min, bb.Max, col_bg2);
		PushStyleColor(ImGuiCol_Text, col_bg);
	}
	else {
		PushStyleColor(ImGuiCol_Text, ImVec4(145 / 255.f, 144 / 255.f, 148 / 255.f, 255.f / 255.f));
	}
	RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
	if (inuse)
	{
		window->DrawList->AddRectFilledMultiColor(ImVec2(bb.Min.x, bb.Max.y), ImVec2(bb.Max.x, bb.Max.y - 20), ImColor(88, 22, 220, 140), ImColor(88, 22, 220, 140), ImColor(88, 22, 220, 0), ImColor(88, 22, 220, 0));
		window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Max.y), ImVec2(bb.Max.x, bb.Max.y - 2), ImColor(88, 22, 220, 200));
	}
	ImGui::PopStyleColor();
...
bool ImGui::BlssButton(const char* label, bool inuse, const ImVec2& size_arg)
{
	return BlssButtonXD(label, size_arg, inuse, 0);
}
...
bool ImGui::KeybindButton(const char* label, const char* unique_id, const ImVec2& size_arg, bool clicked, ImGuiButtonFlags flags)
{
	...
	const ImVec4 hover_dis = ImVec4(79 / 255.f, 8 / 255.f, 160 / 255.f, 1.f);
	...
	const ImVec4 a11_1 = ImVec4(79 / 255.f, 8 / 255.f, 160 / 255.f, 1.f);
	const ImVec4 a11_2 = ImVec4(79 / 255.f, 8 / 255.f, 160 / 255.f, 0.5f);
	const ImVec4 a11_3 = ImVec4(79 / 255.f, 8 / 255.f, 160 / 255.f, 0.f);
	const ImU32 col2 = GetColorU32((held && hovered) ? a11_1 : hovered ? a11_2 : a11_3);

	RenderFrame(bb.Min, bb.Max, col, true, 2.f * c_menu::get().dpi_scale);
	ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(col2), 2.f * c_menu::get().dpi_scale); // IDFK
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags) {
	return ButtonEx_c(label, size_arg, flags, 0);
}
bool ImGui::ButtonEx_c(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, int align)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
...
bool ImGui::CustomButton(const char* label, const char* label_id, const ImVec2& size_arg, bool use_icon, ImFont* icon_font, const char* icon_label)
{
	...
	// Render
	//const ImColor col0 = (held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button;
	//const ImVec4 col1 = ImVec4(col0.Value.x,col0.Value.y,col0.Value.z,col0.Value.w * it_circle->second.alpha);
	const ImU32 col = GetColorU32(ImVec4(30 / 255.f, 30 / 255.f, 30 / 255.f, it_hover->second));

	const ImVec4 text_act = ImVec4(79 / 255.f, 8 / 255.f, 160 / 255.f, 1.f);
	const ImVec4 icon_act = ImVec4(126 / 255.f, 131 / 255.f, 219 / 255.f, 1.f);

	//RenderFrame(bb.Min, bb.Max, col, false, 2.f * c_menu::get().dpi_scale);
	...
	RenderTextClipped(bb.Min + (use_icon ? ImVec2(icon_size.x * 1.2f, 0) : ImVec2(0, 0)), bb.Max, label, NULL, &label_size, style.ButtonTextAlign, &bb);
	PopStyleColor();
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
	...
	// Render
	ImVec4 hover_dis = ImVec4(79 / 255.f, 8 / 255.f, 160 / 255.f, 1.f); // Think it's the scroll bar
	ImVec4 hover_act = ImVec4(79 / 255.f, 8 / 255.f, 160 / 255.f, 1.f); // IDFK
...
bool ImGui::OwOButtonXD(const char* label, const char* icon, ImFont* icon_font, const char* label2, ImFont* label_font, const ImVec2& size_arg, bool inuse, ImGuiWindowFlags flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const ImRect bb(pos, pos + size + ImVec2(0, 15));
	ItemSize(bb, style.FramePadding.y);
	if (!ItemAdd(bb, id))
		return false;
	...
	ImVec4 col_bg;
	if (inuse)
		col_bg = ImLerp(ImVec4(79 / 255.f, 8 / 255.f, 160 / 255.f, 255.f / 255.f), ImVec4(79 / 255.f, 8 / 255.f, 255 / 255.f, 160 / 255.f), t);
	...
	if (icon)
	{
		...
		PushFont(icon_font);
		const ImVec2 icon_size = CalcTextSize(icon, NULL, true);
		RenderText(pos + ImVec2(10, +15), icon);
		ImGui::PopStyleColor();
		PopFont();
	}
	...
	if (label2)
	{
		...
		const ImVec2 icon_size = CalcTextSize(label, NULL, true);
		RenderText(pos - ImVec2(130, size_arg.y / 4 - icon_size.y / 4), label);
		ImGui::PopStyleColor();
		ImGui::NewLine();
	}
	PushFont(label_font);
	RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label2, NULL, &label_size, style.ButtonTextAlign, &bb);
	PopFont();
	ImGui::PopStyleColor();
...
bool ImGui::OwOButton(const char* label, const char* icon, ImFont* icon_font, const char* label2, ImFont* label_font, bool inuse, const ImVec2& size_arg)
{
	return OwOButtonXD(label, icon, icon_font, label2, label_font, size_arg, inuse, 0);
}
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	const ImRect check_bb(pos, pos + ImVec2(square_sz - 3.f, square_sz - 3.f));
	RenderNavHighlight(total_bb, id);
	RenderFrame(check_bb.Min, check_bb.Max, GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true, style.FrameRounding + 5.f);
	ImU32 check_col = GetColorU32(ImGuiCol_CheckMark);
	if (window->DC.ItemFlags & ImGuiItemFlags_MixedValue)
	{
		// Undocumented tristate/mixed/indeterminate checkbox (#2644)
		ImVec2 pad(ImMax(1.0f, IM_FLOOR(square_sz / 3.6f)), ImMax(1.0f, IM_FLOOR(square_sz / 3.6f)));
		window->DrawList->AddRectFilled(check_bb.Min + pad, check_bb.Max - pad, check_col, 2.f);
	}
	else if (*v)
	{
		const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
		RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad - 1.f, pad - 1.f), check_col, square_sz - pad * 2.0f);
	}

	if (g.LogEnabled)
		LogRenderedText(&total_bb.Min, *v ? "[x]" : "[ ]");
	if (label_size.x > 0.0f)
		RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x - 7.f, check_bb.Min.y + style.FramePadding.y), label);
...
static float CalcMaxPopupHeightFromItemCount(int items_count)
{
	...
	if (items_count <= 0)
		return FLT_MAX;
	return (g.FontSize + g.Style.ItemSpacing.y) * min(items_count, 8) - g.Style.ItemSpacing.y + (g.Style.WindowPadding.y * 2);
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int* current_item)
{
	...
	const ImU32 frame_col = GetColorU32(ImVec4(79 / 255.f, 8 / 255.f, 160 / 255.f,1.f));
	//const ImU32 frame_col = GetColorU32(ImVec4(0.f,0.f,0.f,1.f));

	const ImVec4 hover_act = ImVec4(69 / 255.f, 8 / 255.f, 160 / 255.f, 1.f); // IDFK
	const ImVec4 hover_dis = ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 1.f);; // The color around the box tabs 
	...
	const ImVec4 arrow_act = ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 1.f); // outline of when the tab box is active
	const ImVec4 arrow_dis = ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 1.f);; // IDFK
	...
	if ((pressed || g.NavActivateId == id) && !popup_open)
	{
		...
		//PushStyleColor(ImGuiCol_PopupBg, ImVec4(0.f, 0.f, 0.f, 1.f));
	...
	if (!(flags & ImGuiComboFlags_NoArrowButton))
	{
		...
		ImU32 tmpCol = GetColorU32(ImVec4(1.f, 1.f, 1.f, 1.f));
		...
		RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y), tmpCol, it_arrow_alpha->second.dir, 1.0f);
	}


	//window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(it_hover->second), style.FrameRounding);
	window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(hover_act), style.FrameRounding);
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, float power)
{
	...
	const ImVec4 hover_act = ImVec4(79 / 255.f, 8 / 255.f, 160 / 255.f, 1.f); // IDFK
	const ImVec4 hover_dis = ImVec4(79 / 255.f, 8 / 255.f, 160 / 255.f, 0.4f);; // IDFK
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, bool minimum_damage, float power)
{
	...
	const ImU32 frame_col = GetColorU32(ImVec4((79 / 255.f) * 0.1f, (8 / 255.f) * 0.1f, (160 / 255.f) * 0.1f, 1.f));
	const ImVec4 hover_act = ImVec4(79/255.f, 8/255.f, 160/255.f, 1.f); // slider bar i think
	const ImVec4 hover_dis = ImVec4((79 / 255.f) * 0.25f, (8 / 255.f) * 0.25f, (160 / 255.f) * 0.25f, 1.f); // little dot on the scroller 
	//const ImVec4 hover_off = ImVec4(1.f, 0.f, 0.f, 0.4f); // outline of slider
	const ImVec4 hover_off = ImVec4(79 / 255.f, 8 / 255.f, 160 / 255.f, 1.4f); // outline of slider can you make it purple 69,8,160
	...
	//window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImGui::GetColorU32(it_hover->second), g.Style.FrameRounding);
	window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImGui::GetColorU32(hover_off), g.Style.FrameRounding);
	...
	if (grab_bb.Max.x > grab_bb.Min.x)
		window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Max.x + 2.5, frame_bb.Max.y), GetColorU32(ImGuiCol_SliderGrabActive), style.GrabRounding);
	//window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Max.x+2.5,frame_bb.Max.y), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	const ImU32 col_hues[6 + 1] = { IM_COL32(69, 7, 184,style_alpha8), IM_COL32(69, 7, 184,style_alpha8), IM_COL32(0,255,0,style_alpha8), IM_COL32(0,255,255,style_alpha8), IM_COL32(0,0,255,style_alpha8), IM_COL32(255,0,255,style_alpha8), IM_COL32(69, 7, 184,style_alpha8) };
...
bool ImGui::CustomSelectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	//if (g.IO.KeyCtrl) { GetForegroundDrawList()->AddRect(bb_align.Min, bb_align.Max, IM_COL32(69, 7, 184, 255)); }
	...
	const ImVec4 text_act = ImVec4(79 / 255.f, 8 / 255.f, 160 / 255.f, 1.f); // the filler box thing check box
	const ImVec4 text_hov = ImVec4(79 / 255.f, 8 / 255.f, 160 / 255.f, 1.f);
	const ImVec4 text_dis = ImVec4(202 / 255.f, 202 / 255.f, 202 / 255.f, 1.f);
	float deltatime = 1.5f * ImGui::GetIO().DeltaTime;
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	const ImVec4 text_act = ImVec4(79 / 255.f, 8 / 255.f, 160 / 255.f, 1.f);
	const ImVec4 text_hov = ImVec4(1.f, 1.f, 1.f, 1.f);
	const ImVec4 text_dis = ImVec4(1.f, 1.f, 1.f, 1.f);
	float deltatime = 1.5f * ImGui::GetIO().DeltaTime;
	...
	ImVec4 to = ImVec4(0, 0, 0, 0);
	if (hovered || selected)
	{
		to = (hovered && !selected) ? text_hov : text_act;
		if (it_hcolor->second.x != to.x)
		{
			if (it_hcolor->second.x < to.x)
				it_hcolor->second.x = ImMin(it_hcolor->second.x + deltatime, to.x);
	...
	PushStyleColor(ImGuiCol_Text, to);
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
	...
	//window->DrawList->AddRect(ImVec2(tab_bar->BarRect.Max.x - scrolling_buttons_width, tab_bar->BarRect.Min.y), ImVec2(tab_bar->BarRect.Max.x, tab_bar->BarRect.Max.y), IM_COL32(69, 7, 184,255));
