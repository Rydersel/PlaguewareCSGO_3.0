...
float weapon_t::get_inaccuracy()
{
	...
	return engineprediction::get().get_inaccuracy();
	return call_virtual<float(__thiscall*)(void*)>(this, g_ctx.indexes.at(9))(this);
}
...
float weapon_t::get_spread()
{
	if (!this) //-V704
		return 0.0f;
	return engineprediction::get().get_spread();
	return call_virtual<float(__thiscall*)(void*)>(this, g_ctx.indexes.at(10))(this);
}
...
player_info_t player_t::get_info() 
{
	player_info_t info;
	m_engine()->GetPlayerInfo(this->EntIndex(), &info);
	return info;
}
...
bool player_t::is_enemy()
{
	...
	return local->m_iTeamNum() != this->m_iTeamNum();
}
...
bool player_t::is_on_ground()
{
	...
	return m_fFlags() & on_ground || m_fFlags() & partial_ground || m_fFlags() & conveyor;
}

std::optional<Vector> player_t::get_hitbox_position(int id, matrix3x4_t* bones) const
{
	const auto studio_model = m_modelinfo()->GetStudioModel(this->GetModel());
	if (studio_model) {
		auto hitbox_set = studio_model->pHitboxSet(m_nHitboxSet());

		if (!hitbox_set)
			return std::nullopt;

		auto hitbox = hitbox_set->pHitbox(static_cast<uint32_t>(id));
...
int entity_t::m_iTeamNum()
{

	
	return m_iTeamNum_fx();

	//if (m_iTeamNum() != g_ctx.local()->m_iTeamNum())
	//	return true;
	//
	//
	////3 - ct
	////2 - t
	//
	////m_cvar()->FindVar( ( "mp_teammates_are_enemies" )
	//return 0;
}

player_t* Ragdoll::GetPlayer()
{
	return reinterpret_cast<player_t*>(m_entitylist()->GetClientEntityFromHandle(m_hPlayer()));
}
