...
typedef std::array<AnimationLayer, 13> animation_layers;
...
public:
    ...
    NETVAR(int, m_iTeamNum_fx, crypt_str("CBaseEntity"), crypt_str("m_iTeamNum"));
...
public:
    ...
    OFFSET(animation_layers*, get_animation_layers, 0x2980)
...
public:
    ...
    CBaseHandle& m_hPlayer() {
        static int m_hPlayer = netvars::get().get_offset("CCSRagdoll", "m_hPlayer");
        return get< CBaseHandle >(m_hPlayer);
    }

    float& m_flDeathYaw() {
        static int m_flDeathYaw = netvars::get().get_offset("CCSRagdoll", "m_flDeathYaw");
        return get< float >(m_flDeathYaw);
    }

    float& m_flAbsYaw() {
        static int m_flAbsYaw = netvars::get().get_offset("CCSRagdoll", "m_flAbsYaw");
        return get< float >(m_flAbsYaw);
    }
};
