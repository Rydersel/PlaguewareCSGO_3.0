...
void c_menu::draw_legit(int child)
{
	...
	if (!child)
	{
		...
		ImGui::BeginChild(crypt_str("##RAGE1_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
		{
			...
			draw_keybind(crypt_str("Automatic shoot key"), &g_cfg.legitbot.autofire_key, crypt_str("##HOTKEY_AUTOFIRE_LGBT_KEY"));
			ImGui::SliderInt(crypt_str("Automatic shoot delay"), &g_cfg.legitbot.autofire_delay, 0, 12, false, (!g_cfg.legitbot.autofire_delay ? crypt_str("None") : (g_cfg.legitbot.autofire_delay == 1 ? crypt_str("%d tick") : crypt_str("%d ticks"))));
	...
	else
	{
		...
		ImGui::BeginChild(crypt_str("##RAGE1_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
		{
			...
			ImGui::SliderFloat(crypt_str("Smooth amount"), &g_cfg.legitbot.weapon[hooks::legit_weapon].smooth, 0.f, 12.f, crypt_str("%.1f"));
			...
			draw_combo(crypt_str("Recoil Control type"), g_cfg.legitbot.weapon[hooks::legit_weapon].rcs_type, RCSType, ARRAYSIZE(RCSType));
			ImGui::SliderFloat(crypt_str("Recoil Control amount"), &g_cfg.legitbot.weapon[hooks::legit_weapon].rcs, 0.f, 100.f, crypt_str("%.0f%%"), 1.f);

			if (g_cfg.legitbot.weapon[hooks::legit_weapon].rcs > 0)
			{
				ImGui::SliderFloat(crypt_str("Recoil Control Custom FOV"), &g_cfg.legitbot.weapon[hooks::legit_weapon].custom_rcs_fov, 0.f, 30.f, (!g_cfg.legitbot.weapon[hooks::legit_weapon].custom_rcs_fov ? crypt_str("None") : crypt_str("%.2f"))); //-V550
				ImGui::SliderFloat(crypt_str("Recoil Control Custom smooth"), &g_cfg.legitbot.weapon[hooks::legit_weapon].custom_rcs_smooth, 0.f, 12.f, (!g_cfg.legitbot.weapon[hooks::legit_weapon].custom_rcs_smooth ? crypt_str("None") : crypt_str("%.1f"))); //-V550
			}
...
{
	const char* rage_sections[2] = { crypt_str("General"), crypt_str("Accuracy") };
	...
	if (!rage_section)
	{
		if (!child)
		{
			...
			ImGui::BeginChild(crypt_str("##RAGE1_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				ImGui::Checkbox(crypt_str("Enableee"), &g_cfg.ragebot.enable);
				...
				ImGui::Spacing();
				ImGui::Checkbox(crypt_str("Double tape"), &g_cfg.ragebot.double_tap);
				...
				ImGui::Checkbox(crypt_str("Zeus Aimbot"), &g_cfg.ragebot.zeus_bot);
				ImGui::Checkbox(crypt_str("Knife Aimbot"), &g_cfg.ragebot.knife_bot);
		...
		else
		{
			...
			ImGui::BeginChild(crypt_str("##RAGE2_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				else
				{
					...
					{
						ImGui::Checkbox(crypt_str("Direction indicators"), &g_cfg.antiaim.flip_indicator);
						ImGui::SameLine();
	...
	else
	{
		...
		if (!child)
		{
			...
			ImGui::BeginChild(crypt_str("##RAGE1_SECOND"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				draw_combo(crypt_str("Resolver Target selection"), g_cfg.ragebot.weapon[hooks::rage_weapon].selection_type, selection, ARRAYSIZE(selection));
				padding(0, 3);
		...
		else
		{
			...
			ImGui::BeginChild(crypt_str("##RAGE2_SECOND"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				if (g_cfg.ragebot.weapon[hooks::rage_weapon].hitchance)
					ImGui::SliderInt(crypt_str("Hitchance amount"), &g_cfg.ragebot.weapon[hooks::rage_weapon].hitchance_amount, 0, 100);

				ImGui::SliderInt(crypt_str("Minimum visible damage"), &g_cfg.ragebot.weapon[hooks::rage_weapon].minimum_visible_damage, 0, 120, true);
...
{
	const char* visuals_sections[3] = { crypt_str("General"), crypt_str("Viewmodel"), ("Skin changer") };
...
void c_menu::draw_visuals(int child)
{
		...
		if (!child)
		{
			if (!visuals_section)
			{
				...
				ImGui::BeginChild(crypt_str("##VISUAL1_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
				{
					...
					ImGui::Checkbox(crypt_str("World effects"), &g_cfg.esp.world_modulation);
		...
		else
		{
			if (!visuals_section)
			{
				...
				ImGui::BeginChild(crypt_str("##VISUAL1_SECOND"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
				{
					...
					ImGui::Checkbox(crypt_str("Fog changer"), &g_cfg.esp.fog);
	...
	else if (child == -1)
	{
		...
		ImGui::BeginChild(crypt_str("##SKINCHANGER__TAB"), ImVec2(686 * dpi_scale, child_height * dpi_scale)); // first functional child
		{
			...
			tab_start();
			ImGui::BeginChild(crypt_str("##SKINCHANGER__CHILD"), ImVec2(6416 * dpi_scale, (child_height - 35) * dpi_scale));
			{
...
void c_menu::draw_players(int child)
{
	...
	if (!child)
	{
		...
		ImGui::BeginChild(crypt_str("##ESP1_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
		{
			...
			if (player == ENEMY)
			{
				ImGui::Checkbox(crypt_str("Out Of Fov Indicator"), &g_cfg.player.arrows);
				ImGui::SameLine();
		...
		ImGui::BeginChild(crypt_str("##ESP2_SECOND"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
		{
			...
			if (g_cfg.player.type[player].chams[PLAYER_CHAMS_VISIBLE] || player == LOCAL && g_cfg.player.local_chams_type) //-V648
			{
				if (player == LOCAL && g_cfg.player.local_chams_type)
				{
					...
					{
						ImGui::Checkbox(crypt_str("Double chams "), &g_cfg.player.fake_double_material);
						ImGui::SameLine();
...
{
	const char* misc_sections[4] = { crypt_str("Main"), crypt_str("Additives"), "Configs", "Luas" };
	draw_semitabs(misc_sections, 4, misc_section, ImGui::GetStyle());
}
...
void c_menu::draw_misc(int child)
{
	if (!child)
	{
		if (!misc_section)
		{
			...
			{
				ImGui::Checkbox(crypt_str("Anti-untrusteed"), &g_cfg.misc.anti_untrusted);
		...
		else
		{
			...
			ImGui::BeginChild(crypt_str("##MISC1_SECOND"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				//ImGui::Checkbox(crypt_str("RGB Watermark"), &g_cfg.menu.rainbowwatermark);
	...
	else
	{
		if (!misc_section)
		{
			...
			ImGui::BeginChild(crypt_str("##MISC2_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				//draw_keybind(crypt_str("Edge bug"), &g_cfg.misc.edge_bug, crypt_str("##EDGEBUG__HOTKEY"));
		...
		ImGui::BeginChild(crypt_str("##LUA_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
		{
			...
			if (ImGui::CustomButton(crypt_str("Open scripts folder"), crypt_str("##LUAS__FOLDER"), ImVec2(291 * dpi_scale, 26 * dpi_scale)))
			{
				...
				auto get_dir = [&folder]() -> void
				{
					...
					if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
						folder = std::string(path) + crypt_str("\\Plagueware\\Scripts\\");
