...
bool aim::automatic_stop(CUserCmd* cmd)
{
	...
	{
		if (!g_cfg.add_shonax.autostop_fixer)
		{
			Vector direction;
			Vector real_view;
		...
		else
		{
			...
			{
				cmd->m_sidemove += +left.y;
			}

			if (!(cmd->m_buttons & IN_MOVERIGHT))
			{
				cmd->m_sidemove -= -right.y;
			}

			if (!(cmd->m_buttons & IN_FORWARD))
			{
				if (cmd->m_buttons & IN_MOVELEFT || cmd->m_buttons & IN_MOVERIGHT || g_cfg.misc.airstrafe)
					return false;

				cmd->m_forwardmove += +move_forward.x;
			}
			...
			if (!(cmd->m_buttons & IN_BACK))
			{
				...
				cmd->m_forwardmove -= -move_backward.x;
			}
		}
...
int aim::hitchance(const Vector& aim_angle)
{
	...
	if (g_ctx.globals.weapon && g_cfg.add_shonax.hitchance_fixer /*&& (g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_AWP || g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_SSG08)*/)
	{
		auto final_hitchance = 0;
		auto weapon_info = g_ctx.globals.weapon->get_csweapon_info();
	...
	auto damage = 0;
	auto high_accuracy_weapon = !g_cfg.add_shonax.hitchance_fixer && (g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_AWP || g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_SSG08);
