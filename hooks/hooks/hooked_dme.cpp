...
void __stdcall hooks::hooked_dme(IMatRenderContext* ctx, const DrawModelState_t& state, const ModelRenderInfo_t& info, matrix3x4_t* bone_to_world)
{
	...
	auto is_weapon = strstr(name, "weapons/v_") && !strstr(name, "arms") && (g_cfg.esp.weapon_chams || g_cfg.add_shonax.customchams);
	...
	if (m_engine()->IsConnected() && m_engine()->IsInGame() && g_cfg.add_shonax.customchams)
	{
		...
		knowledge.is_glove = strstr(info.pModel->name, "glove") != nullptr;
		//knowledge.draw_skins(original);
	}
	...
	if (g_cfg.add_shonax.customchams)
	{

		if (g_ctx.globals.weapon && g_ctx.globals.weapon->is_knife())
		{
			auto alpha = 1.0f;
	...
	CreateMaterial(true, crypt_str(R"#("VertexLitGeneric" 
			...
			auto material = materials_knife_custom[g_cfg.add_shonax.customknife];
			auto double_material = materials[6];

			if (material && double_material && !material->IsErrorMaterial() && !double_material->IsErrorMaterial() && knowledge.is_knife)
			{
				float weapon_color[3] =
				{
					g_cfg.add_shonax.knife_color_custom[0] / 255.0f,
					g_cfg.add_shonax.knife_color_custom[1] / 255.0f,
					g_cfg.add_shonax.knife_color_custom[2] / 255.0f
				};
