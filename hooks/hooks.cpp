...
namespace hooks
{
	...
	LPDIRECT3DTEXTURE9 skinImg = nullptr;
	...
	void GUI_Init(IDirect3DDevice9* pDevice)
	{
		...
		// Menu fonts
		c_menu::get().futura_small = io.Fonts->AddFontFromMemoryTTF((void*)futuramediumc, sizeof(futuramediumc), 12.f, &m_config, ranges);
		c_menu::get().futura = io.Fonts->AddFontFromMemoryTTF((void*)futuramediumc, sizeof(futuramediumc), 17.f, &m_config, ranges);
		c_menu::get().futura_large = io.Fonts->AddFontFromMemoryCompressedTTF(newfont_compressed_data, newfont_compressed_size, 20.f);
		c_menu::get().gotham = io.Fonts->AddFontFromMemoryCompressedTTF(IconsFont_compressed_data, IconsFont_compressed_size, 25.f);

		if (skinImg == nullptr)
			D3DXCreateTextureFromFileInMemoryEx(pDevice, &myFile, sizeof(myFile), 500, 550, D3DX_DEFAULT, D3DUSAGE_DYNAMIC, D3DFMT_UNKNOWN, D3DPOOL_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, &skinImg);
	...
	long __stdcall hooked_present(IDirect3DDevice9* device, RECT* src_rect, RECT* dest_rect, HWND dest_wnd_override, RGNDATA* dirty_region)
	{
		...
		//for (static auto i = 0; i < 1; skeet_menu.steam_image(), i++);
